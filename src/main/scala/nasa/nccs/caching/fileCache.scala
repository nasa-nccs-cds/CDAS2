...
import nasa.nccs.cdas.engine.spark.{RangePartitionKey, RangePartitionKey$, RangePartitioner$, TimePartitionKey}
...
class Partition(val index: Int, val path: String, val dimIndex: Int, val startIndex: Int,
  ...
  def getPartitionKey( grid: TargetGrid ): RangePartitionKey = TimePartitionKey( grid.getCalendarDate(origin(0)+startIndex), grid.getCalendarDate(origin(0)+startIndex+partSize), partSize )
