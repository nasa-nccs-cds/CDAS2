...
object NCMLWriteManager extends Loggable {

  def writeNetCDFAggregation( pathFile: File, ncmlFile: File, collectionId: String ) = {
    getNcFile( pathFile ) match {
      case Some( sampleFile ) =>
        val metaData = FileMetadata( sampleFile.toURI )
            ...
            val aggElem = <?xml version="1.0" encoding="UTF-8"?>
              <netcdf xmlns="http://www.unidata.ucar.edu/namespaces/netcdf/ncml-2.2">
                ...
                <aggregation type="union">
                  <scan location={pathFile.toURI.toString} subdirs="true" suffix={extension}/>
                </aggregation>
              </netcdf>

            val bw = new BufferedWriter(new FileWriter( ncmlFile ))
            bw.write( aggElem.toString )
            bw.close()

//            val ncd: NetcdfDataset  = new NetcdfDataset()
//            val agg = new ncml.AggregationUnion( ncd, timeAxis.getShortName, "" )
//            val enhanceMode = new java.util.HashSet[NetcdfDataset.Enhance]( )
//            for ( emval <- List( NetcdfDataset.Enhance.CoordSystems, NetcdfDataset.Enhance.ConvertEnums ) ) enhanceMode.add( emval )
//            agg.addDatasetScan( null, pathFile.toString, null, "", null, enhanceMode, "true", null )
//            val writer = new ncml.NcMLWriter()
//            agg.finish(cancel)
//            writer.writeXML(ncd)
          case None => throw new Exception( "Can't find time axis in data file: " + sampleFile )
        }
      case None => throw new Exception( "Error, empty collection at: " + pathFile.toString )
    }
  }
  ...
  def getNcFile(file: File): Option[File] = {
    try {
      if (isNcFile(file)) Some(file )
      else  {
        val children = new Iterable[File] {
          def iterator = if (file.isDirectory) file.listFiles.iterator else Iterator.empty
        }
        for( child <- children; result = getNcFile( child ) )  { if(result.isDefined) return result }
        None
      }
    } catch {
      case err: NullPointerException =>
  ...
  def getNcFiles(args: Iterator[File]): Iterator[File] = args.map( (arg: File) => getNcFiles(arg)).flatten
  def getNcURIs(args: Iterator[File]): Iterator[URI] = args.map( (arg: File) => getNcURIs(arg)).flatten
...
//  val files: IndexedSeq[File] = NCMLWriter1.getNcFiles(args).toIndexedSeq
//  val nFiles = files.length
//  val fileHeaders = NCMLWriter1.getFileHeadersSerial(files)
//
...
//}
//
//class NCMLWriter1(args: Iterator[File], val maxCores: Int = 8) extends Loggable {
//  import NCMLWriteManager._
//  private val nReadProcessors = Math.min(Runtime.getRuntime.availableProcessors, maxCores)
//  private val files: IndexedSeq[URI] = getNcURIs(args).toIndexedSeq
...
//  val fileHeaders = getFileHeaders(files, nReadProcessors)
