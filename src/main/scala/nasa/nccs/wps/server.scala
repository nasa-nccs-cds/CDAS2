  ...
  def GetCapabilities: xml.Elem =
    <wps:Capabilities service="WPS" version="1.0.0" xml:lang="en-CA" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:wps="http://www.opengis.net/wps/1.0.0" xmlns:ows="http://www.opengis.net/ows/1.1" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.opengis.net/wps/1.0.0 ../wpsGetCapabilities_response.xsd" updateSequence="1">
      <ows:ServiceIdentification>
        <ows:Title> {appParameters("wps.server.title", "Climate Data Analytics Server (CDAS)")}  </ows:Title>
        <ows:Abstract> {appParameters("wps.server.abstract", "High Performance Spark-based Climate Data Analytics delivered via the ESGF Compute Working Team WPS API")} </ows:Abstract>
        <ows:Keywords>
      ...
      <ows:ServiceProvider>
        <ows:ProviderName> {appParameters("wps.server.provider.name", "NASA NCCS")} </ows:ProviderName>
        <ows:ProviderSite xlink:href={appParameters("wps.server.provider.url", "https://www.nccs.nasa.gov")}/>
        <ows:ServiceContact>
          <ows:IndividualName>  {appParameters("wps.server.manager.name", "Thomas Maxwell")} </ows:IndividualName>
          <ows:PositionName>  {appParameters("wps.server.manager.title", "Lead System Architect")} </ows:PositionName>
          <ows:ContactInfo>
            <ows:Phone>  <ows:Voice>  {appParameters("wps.server.manager.phone", "301-286-7810")}  </ows:Voice>   </ows:Phone>
            <ows:Address> <ows:ElectronicMailAddress> {appParameters("wps.server.manager.email", "thomas.maxwell@nasa.gov")}   </ows:ElectronicMailAddress>  </ows:Address>
          </ows:ContactInfo>
        </ows:ServiceContact>
      </ows:ServiceProvider>
