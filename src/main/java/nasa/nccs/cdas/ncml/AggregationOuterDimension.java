...
public abstract class AggregationOuterDimension extends nasa.nccs.cdas.ncml.Aggregation implements ProxyReader {
    ...
    protected List<nasa.nccs.cdas.ncml.AggregationOuterDimension.CacheVar> cacheList = new ArrayList<nasa.nccs.cdas.ncml.AggregationOuterDimension.CacheVar>(); // promote global attribute to variable
    ...
    void addVariableFromGlobalAttribute(String varName, String orgName) {
        cacheList.add(new nasa.nccs.cdas.ncml.AggregationOuterDimension.PromoteVar(varName, orgName));
    }
    ...
    void addVariableFromGlobalAttributeCompose(String varName, String format, String gattNames) {
        cacheList.add(new nasa.nccs.cdas.ncml.AggregationOuterDimension.PromoteVarCompose(varName, format, gattNames));
    }
    ...
    void addCacheVariable(String varName, DataType dtype) {
        ...
        cacheList.add(new nasa.nccs.cdas.ncml.AggregationOuterDimension.CacheVar(varName, dtype));
    }

    nasa.nccs.cdas.ncml.AggregationOuterDimension.CacheVar findCacheVariable(String varName) {
        for (nasa.nccs.cdas.ncml.AggregationOuterDimension.CacheVar cv : cacheList)
    ...
    protected void buildCoords(CancelTask cancelTask) throws IOException {
        ...
        if (type == Type.forecastModelRunCollection) {
            for (Dataset nested : nestedDatasets) {
                nasa.nccs.cdas.ncml.AggregationOuterDimension.DatasetOuterDimension dod = (nasa.nccs.cdas.ncml.AggregationOuterDimension.DatasetOuterDimension) nested;
                dod.ncoord = 1;
            }
        }
    ...
    protected void promoteGlobalAttributes(nasa.nccs.cdas.ncml.AggregationOuterDimension.DatasetOuterDimension typicalDataset) throws IOException {

        for (nasa.nccs.cdas.ncml.AggregationOuterDimension.CacheVar cv : cacheList) {
            if (!(cv instanceof nasa.nccs.cdas.ncml.AggregationOuterDimension.PromoteVar)) continue;
            nasa.nccs.cdas.ncml.AggregationOuterDimension.PromoteVar pv = (nasa.nccs.cdas.ncml.AggregationOuterDimension.PromoteVar) cv;
    ...
    public Array reallyRead(Variable mainv, Section section, CancelTask cancelTask) throws IOException, InvalidRangeException {
        ...
        if (spObj != null && spObj instanceof nasa.nccs.cdas.ncml.AggregationOuterDimension.CacheVar) {
            nasa.nccs.cdas.ncml.AggregationOuterDimension.CacheVar pv = (nasa.nccs.cdas.ncml.AggregationOuterDimension.CacheVar) spObj;
    ...
    public Array reallyRead(Variable mainv, CancelTask cancelTask) throws IOException {
        ...
        List<Dataset> nestedDatasets = getDatasets();
        if (executor != null) {
            CompletionService<nasa.nccs.cdas.ncml.AggregationOuterDimension.Result> completionService = new ExecutorCompletionService<>(executor);

            int count = 0;
            for (Dataset vnested : nestedDatasets)
                completionService.submit(new nasa.nccs.cdas.ncml.AggregationOuterDimension.ReaderTask(vnested, mainv, cancelTask, count++));

            try {
                int n = nestedDatasets.size();
                for (int i = 0; i < n; ++i) {
                    nasa.nccs.cdas.ncml.AggregationOuterDimension.Result r = completionService.take().get();
                    if (r != null) {
    ...
    private static class ReaderTask implements Callable<nasa.nccs.cdas.ncml.AggregationOuterDimension.Result> {
        Dataset ds;
        Variable mainv;
        ...
        public nasa.nccs.cdas.ncml.AggregationOuterDimension.Result call() throws Exception {
            Array data = ds.read(mainv, cancelTask);
            return new nasa.nccs.cdas.ncml.AggregationOuterDimension.Result(data, index);
        }
    }
  ...
  private void readAggCoord(Variable aggCoord, CancelTask cancelTask, DatasetOuterDimension vnested, DataType dtype, IndexIterator result,
    ...
    protected Dataset makeDataset(String cacheName, String location, String id, String ncoordS, String coordValueS, String sectionSpec,
        ...
        return new nasa.nccs.cdas.ncml.AggregationOuterDimension.DatasetOuterDimension(cacheName, location, id, ncoordS, coordValueS, enhance, reader);
    }

    @Override
    protected Dataset makeDataset(MFile dset) {
        return new nasa.nccs.cdas.ncml.AggregationOuterDimension.DatasetOuterDimension(dset);
    }
    ...
    protected void cacheCoordValues(NetcdfFile ncfile) throws IOException {
      ...
      if (coordVar != null) {
        ...
        protected void cacheVariables(NetcdfFile ncfile) throws IOException {
            for (nasa.nccs.cdas.ncml.AggregationOuterDimension.CacheVar pv : cacheList) {
                pv.read(this, ncfile);
            }
        }
        ...
        public int compareTo(Object o) {
            if (coordValueDate == null)
                ...
                nasa.nccs.cdas.ncml.AggregationOuterDimension.DatasetOuterDimension other = (nasa.nccs.cdas.ncml.AggregationOuterDimension.DatasetOuterDimension) o;
    ...
    class CacheVar {
        ...
        Array read(Section section, CancelTask cancelTask) throws IOException, InvalidRangeException {
            ...
            List<Dataset> nestedDatasets = getDatasets();
            for (Dataset vnested : nestedDatasets) {
                nasa.nccs.cdas.ncml.AggregationOuterDimension.DatasetOuterDimension dod = (nasa.nccs.cdas.ncml.AggregationOuterDimension.DatasetOuterDimension) vnested;
        ...
        protected Array read(nasa.nccs.cdas.ncml.AggregationOuterDimension.DatasetOuterDimension dset) throws IOException {
        ...
        protected Array read(nasa.nccs.cdas.ncml.AggregationOuterDimension.DatasetOuterDimension dset, NetcdfFile ncfile) throws IOException {
            invocation++;
    ...
    class CoordValueVar extends nasa.nccs.cdas.ncml.AggregationOuterDimension.CacheVar {
        String units;
        DateUnit du;
        ...
        private Array readCached(nasa.nccs.cdas.ncml.AggregationOuterDimension.DatasetOuterDimension dset) throws IOException {
            Array data = getData(dset.getId());
            if (data != null) return data;
    ...
    class PromoteVar extends nasa.nccs.cdas.ncml.AggregationOuterDimension.CacheVar {
        String gattName;
    ...
    /////////////////////////////////////////////
    // global attributes promoted to variables
    class PromoteVarCompose extends nasa.nccs.cdas.ncml.AggregationOuterDimension.PromoteVar {
        String format;
        String[] gattNames;
