...
public class PythonWorker {
    ...
    public class ResultThread extends Thread {
        ...
        public ResultThread( int result_port, ZMQ.Context context ) {
            ...
            result_socket.connect("tcp://*:"+ String.valueOf(result_port));
        }
        public void run() {
            while( active ) try {
    ...
    public PythonWorker( int worker_index, ZMQ.Context context ) {
        ...
        process = startup( worker_index, request_port, result_port );
        resultThread = new ResultThread( result_port, context );
        resultThread.start();
    }

    public void sendDataPacket( String header, byte[] data ) {
        request_socket.send(header, 0);
        request_socket.send(data, 0);
    }

    public void sendArrayData( String id, int[] origin, int[] shape, byte[] data, Map<String, String> metadata ) {
        String header = "|".join( "array", id, ia2s(origin), ia2s(shape), m2s(metadata) );
        sendDataPacket( header, data );
    }

    public void sendRequest( String operation, String[] inputs, HashMap<String, String> metadata ) {
        String header = "|".join( "task", operation, s2s(inputs), m2s(metadata) );
        request_socket.send(header, 0);
    }

    private Process startup( int worker_index, int request_port, int result_port ) {
        try {
            String cdas_home = System.getenv("CDAS_HOME_DIR");
            Path path = FileSystems.getDefault().getPath(cdas_home, "python", "pycdas", "icdas.py" );
            Process process = new ProcessBuilder( "python", path.toString(), String.valueOf(worker_index), String.valueOf(request_port), String.valueOf(result_port) ).start();
    ...
    public String s2s( String[] array ) { return Arrays.toString(array).replaceAll("\\[|\\]|\\s", ""); }
